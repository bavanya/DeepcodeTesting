...
class Sudoku(object):
    ...
    def __setitem__(self, p, val):
        ...
        pb = (p[0] // 3, p[1] // 3)
        block = self._field[pb[1] + 3 * pb[0]]block[p[0] % 3, p[1] % 3] = val
...
class CursesUI(object):
    def __init__(self):
        ...
        self._sudoku = Sudoku()
        # Draw the borders
        self._screen.vline(0, 4 * 3 - 2, curses.ACS_VLINE, 2 * 9 - 1)self._screen.vline(0, 4 * 6 - 2, curses.ACS_VLINE, 2 * 9 - 1)
    ...
    def _help(self):
        self._print_string("Commands: (q)uit, (p)opulate, (P)opulate, ""(r)eset, (c)andidates, (s)olve")
    ...
    def main(self):
        ...
        while True:
            ...
            if c == 'q':
                ...
                self._draw_sudoku()
                self._print_string("Populated with %d fields in %.3f seconds" % (n, end - start))
            elif c == 's':
                if not self._sudoku.solve():
                    elf._print_string("Could not solve sudoku")
                self._draw_sudoku()
            elif c == 'r':
                self._sudoku = Sudoku()
                self._draw_sudoku()
